  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">Attendance Overview</h1>
          <p className="text-xs text-slate-500">Semester 5 Session Attendance Records (Mandatory minimum: 85%)</p>
        </div>
        ...
        <CardContent className="overflow-x-auto p-0">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {mockCourses.map((c, i) => (
                <tr key={c.code} className="hover:bg-slate-50/75">
